(function(root) {
  ...
  var Game = AsteroidsGame.Game = function Game(ctx) {
    ...
    this.ship = new AsteroidsGame.Ship([Game.SHIP1_POS[0], Game.SHIP1_POS[1]], 1, Game.SHIP1_COLOR);
	  this.ship2 = new AsteroidsGame.Ship([Game.SHIP2_POS[0], Game.SHIP2_POS[1]], 2, Game.SHIP2_COLOR);
    this.bullets = [[],[],[]];
    this.score = 0;
...
// KEY HANDLERS //
  ...
  Game.prototype.bindKeyHandlers = function() {
    var that = this;
    // key("up", function ()    { that.ship.power([0,-5])});
    // key("down", function ()  { that.ship.power([0,5])});
    // key("left", function ()  { that.ship.power([-5,0])});
    // key("right", function () { that.ship.power([5,0])});
    //
    key("space", function () { that.fireBullet(that.ship) });
    //
    // key("w", function () { that.ship2.power([0,-5])});
    // key("s", function () { that.ship2.power([0,5])});
    // key("a", function () { that.ship2.power([-5,0])});
    // key("d", function () { that.ship2.power([5,0])});
    //
